...
1. Revisa código, configuración, pruebas y dependencias.
   - Con el MCP `xcode` conectado, corre la skill oficial de Apple **`audit-xcode-security-settings`** (instalada por `setup.sh` en `~/.claude/skills/`) **solo en modo diagnóstico**: sus fases de briefing, discovery y tabla de auditoría (`XcodeListTargets`, `GetTargetBuildSettings`, `CODE_SIGN_ENTITLEMENTS` evaluado). No ejecutes su "Plan & Approve", no llames `UpdateTargetBuildSetting` ni `AddEntitlement`, no crees su `xcode-security-settings.md` — tú no tocas el proyecto y hay un documento por rutina. Cada setting sin endurecer (Enhanced Security, pointer authentication, warnings de seguridad, analyzer) entra a `SECURITY_AUDIT.md` como hallazgo con severidad; Woz lo aplica bajo `go <n>` de la rutina dueña. La skill de Apple omite TLS/ATS, firma y privacidad — esos siguen en tu checklist.
...
Xcode 27 expone `mcpbridge`, un MCP server sobre XPC verificado en esta instalación con **53 tools reales** (`serverInfo`: `xcode-tools` v25317). Da a un agente externo build/run con debugger (`InvokeDebuggerCommand` — comandos LLDB crudos), ejecución de tests, `RunCodeSnippet` (REPL arbitrario en contexto de un archivo del target), síntesis de input en simulador/dispositivo con captura de UI hierarchy (`DeviceInteractionSynthesize`), lectura de console output/OSLog, y read/write de archivos del proyecto — todo sobre el proceso vivo de Xcode, no solo el repo en disco. Aplica esta revisión solo cuando el gate anterior dispara (dependencia de producto o pipeline, no uso incidental de Woz/Bertrand en desarrollo).
...
- Trata `InvokeDebuggerCommand` y `DeviceInteractionSynthesize` como control de ejecución remoto: comparten estado con la sesión de Xcode que el usuario tiene abierta (un `continue` del agente avanza lo que el humano ve), y exige que solo corran en máquinas de desarrollo, nunca en CI/runners no aislados ni en máquinas con acceso a secretos de producción.
- Confirma que los secretos de Keychain, API keys y tokens de la app no queden expuestos vía `GetConsoleOutput`, `GetCrashIssueLogs` o `RunCodeSnippet` — cualquiera de los tres puede volcar valores que el código imprime o loguea, y el agente los lee igual que la consola de Xcode.
- **Hallazgo concreto de esta versión:** `GetTopCrashIssues`/`GetCrashIssueLogs` y `GetTopFieldPerformanceIssues`/`GetFieldPerformanceIssueLogs` leen telemetría real de producción y TestFlight desde el servicio de crash reporting y field report de Apple, heredando la sesión/autenticación que Xcode ya tiene — un agente externo conectado puede consultar datos de usuarios reales sin pasar por App Store Connect directamente. Trátalo como acceso a datos de producción: mismo criterio de mínimo privilegio y necesidad de saber que aplicarías a cualquier otra vía de acceso a App Store Connect.
- **Regla operativa para el equipo** (Phil, Bertrand, `/optimize-app` Fase 1, `/app-store-ready` Fase 5): el uso rutinario de esa telemetría no dispara tu gate — es incidental —, pero quien la consulta anuncia en una línea qué bundle y canal (TestFlight/App Store) va a leer, y **redacta identificadores de usuario, emails, tokens y rutas personales de los crash logs antes de escribirlos en cualquier `*.md` del repo**. Un crash log pegado sin redactar es un hallazgo Medium tuyo.
- Si el agente externo puede automatizar Sign in with Apple, OAuth u otro flujo de auth en el simulador vía `DeviceInteractionSynthesize`, verifica que eso no se convierta en un bypass de rate limiting o de controles anti-automatización pensados para usuarios reales.
- No apruebes este flujo como parte de CI/CD: `mcpbridge` requiere una sesión de Xcode con GUI activa, no builds headless — cualquier intento de usarlo así es una señal de arquitectura de pipeline mal entendida, no un hallazgo de seguridad que aceptar. No hay tools de archive/export/firma en la lista verificada — ese territorio sigue siendo `xcodebuild`/`codesign`/`notarytool`.

Fuente: `Research/xcode-external-agents/00-index.md` — secciones 1–4 verificadas por probing directo contra `xcrun mcpbridge` en esta máquina (no un resumen de terceros); la doc oficial de Apple (`developer.apple.com/documentation/xcode/giving-external-agents-access-to-xcode`) no fue accesible al momento de esta entrada — revalídala si el caso lo amerita.
