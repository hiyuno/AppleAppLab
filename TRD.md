...
1. `PeriodDateEngine` calcula el rango de fechas de cualquier `(year, month, half)` sin tocar el store — es una función pura. La navegación "quincena anterior/siguiente" avanza sobre estas coordenadas, no sobre registros.
   - **Límite histórico:** `PeriodDateEngine` no genera ni permite navegar a coordenadas anteriores a la primera `Period` materializada (el punto donde el usuario empezó a usar la app). No hay quincenas "hacia atrás" — la navegación a "anterior" se detiene ahí; no es una proyección simétrica hacia el pasado.
